...
import { View, Text, ScrollView, Image, StyleSheet, Pressable, Modal, Share } from 'react-native'
  ...
  async function shareRecipe() {
    try {
      await Share.share({
        ...
        message: route.params?.data.video
      })
    } catch (error) {
      console.log(error)
    }
  ...
  return (
    ...
    >
      ...
      {route.params?.data.ingredients.map((item) => (
        <Ingredients key={item._id} data={item} />
      ))}
      ...
      {route.params?.data.instructions.map((item, index) => (
        <Instructions key={item._id} data={item} index={index} />
      ))}
